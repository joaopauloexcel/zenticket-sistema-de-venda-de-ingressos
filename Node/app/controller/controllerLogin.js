...
exports.signin = (req, res) => {
	if(req.body.email){
		People.findOne({//procura  no banco de dados primeiro usuario
			...
			var passwordIsValid = bcrypt.compareSync(req.body.senha, people.senha);
			if (!passwordIsValid) {
			...
			if(people.permissao){
				var authorities = people.permissao;//instancia variavel que armazenará as autorizações dos usuários	
...
exports.recoverEmail = (req, res) => {
	if(req.body.email){
		People.findOne({//procura  no banco de dados primeiro usuario
			...
			people.update({
				senha: bcrypt.hashSync(newPassword, 8)
			})
			const mailOptions = {
				from: `"${people.nome}" <${email}>`,
				to: `${people.email}`,
				subject: 'ZenTicket - Solicitação de restauração de senha!',
				...
				<br><br><br><br><b><i>Equipe - ZenTicket</i></b></p>`
			  };
